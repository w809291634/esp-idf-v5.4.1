...
uint8_t DRAM_ATTR lvgl_draw_buf1[DRAW_BUFFER_SIZE] __attribute__((aligned(16)));
uint8_t DRAM_ATTR lvgl_draw_buf2[DRAW_BUFFER_SIZE] __attribute__((aligned(16)));

// 检查是否对齐（可选）
_Static_assert(((uintptr_t)lvgl_draw_buf1 % 16) == 0, "lvgl_draw_buf1 not aligned");
_Static_assert(((uintptr_t)lvgl_draw_buf2 % 16) == 0, "lvgl_draw_buf2 not aligned");
#endif
...
void lvgl_init(void)
{
    ...
    void *lvgl_draw_buf1 = spi_bus_dma_memory_alloc(LCD_HOST, draw_buffer_sz, 0);
    assert(lvgl_draw_buf1);
    void *lvgl_draw_buf2 = spi_bus_dma_memory_alloc(LCD_HOST, draw_buffer_sz, 0);
    assert(lvgl_draw_buf2);
#endif
    // initialize LVGL draw buffers
    lv_display_set_buffers(display, lvgl_draw_buf1, lvgl_draw_buf2, draw_buffer_sz, LV_DISPLAY_RENDER_MODE_PARTIAL);
